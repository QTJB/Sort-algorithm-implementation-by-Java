...
//  Setting start condition through Recursive algorithm nextPermutation.
...
//  Recursive algorithm nextPermutation.
	...
	/*
	 * If (a1,…,an) is an arbitrary permutation of the numbers 1,2,…,n, then:  If
	 * the maximal element of the permutation (which is n) is not the first element
	 * of the permutation, say, n = a(i), where i > 1, then swap a(i) and a(i-1) and
	 * return true(1).  If the maximal element of the permutation is in the first
	 * position, i.e. n = a1, call nextPermutation algorithm for the smaller
	 * (n-1)-element permutation (a2,…,an) (this is a recursive step!) and:  if
	 * this call returns true (1), append a1 to the obtained permutation of (n-1)
	 * elements and return true (1).  else return false (0).
	 */
...
//  Iterative algorithm nextPermutation.
	 ...
	 * 
	 * 
	 * The function of following are for Insertion, merge and quick sort.
	 */
...
//			To calculate the number of element comparisons.
			for (int j = tempIndex + 1; j > 0
					&& comparisonWithCounters(list[tempIndex], list[j], 1, false); --j, --tempIndex) {
	...
	private static void mergeSortPart(int[] list, int first, int last) {
		if (last > first) {
	...
	private static void merge(int[] list, int first, int mid, int last) {
		...
		while (last1 >= first1 && last2 >= first2) {
			if (comparisonWithCounters(list[first2], list[first1], 2, false)) {
	...
	private static void quickSortPart(int[] list, int left, int right) {
	...
	private static int partition(int[] list, int left, int right) {
		...
		while (true) {
			while (left < right && comparisonWithCounters(list[right], list[left], 3, false)) {
				right--;
			}
			if (left < right) {
...
//	Using insertion sort.
	private static void insertionSortToMatrix() {
		...
		for (int i = 0; i <= matrixRecursive.length - 1; ++i) {
			insertionSort(matrixRecursive[i]);
		}
...
//	Calculating the total number of all permutations possibility.
	 ...
	 * Quick sort
	 */
	private static boolean comparisonWithCounters(int firstElement, int secondElement, int typeOfSort, boolean equal) {
